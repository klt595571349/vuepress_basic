...
```js
const { age, username } = { username: "假城", age: 18 };
```

**完整写法**

```js
const { age: age, username: username } = { username: "假城", age: 18 };
//属性名引号可以省略
```
...
```js
const { age: age, username: uname } = { username: "假城", age: 18 };
...
```js
const { username = "ZhangSan", age = 18 } = { username: "假城", age: 58 };
...
logPersonInfo({ username: "假城", age: 18 });
```
...
```js
const logPersonInfo = ({ username = "假城", age = 18 }) => {
  console.log(username, age);
};
...
> 数组成员/对象的属性值严格等于(===) undefined 时，对应的默认值才会生效
>
> 如果默认值是表达式，默认值表达式是惰性求值的
